...
<input name='invoice_id' id="invoice_id" type='hidden' value='{$INVOICE_ID}' />
</form>
...
{literal}
	...
	jQuery('input[name*="subproduct_ids"]').each(function(e) {				
		if(jQuery(this).val() !='') {
			jQuery(this).parent().parent().parent().find('tr:gt(0):last').remove();
		}
	});	
//	subproduct_ids3
{/literal}
