...
$temp_me = rtrim('http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']),"/");

// CRM URL without out trialing /
// Example: http://yourdomain.com/crm
$Server_Path = rtrim(str_replace('customerportal','',$temp_me),"/");

// Portal URL without trialing /
// Example: http://yourdomain.com/portal
$Authenticate_Path = $temp_me;
