	...
	{if $PRODUCT_EDITABLE}
		<td>
			...
			{else}
				...
				{/if}
			{/if}
			<button type="button" class="btn btn-default" id="addSN" >&nbsp;&nbsp;<strong>Add SN</strong></button>
			<div class="AddSN validCheck hide" id="discount_div0">
			    <input type="hidden" id="discount_type0" name="discount_type0" value="zero" class="discount_type">
			    <p class="popover_title">Serial Number</p>
			    <table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-nobordered popupTable">
			        <tbody>
		        		<tr>
			                <td><input type="text" value="123456789" id="serial_number113" name="serial_number113" size="20" class="small"></td>
			                <td>
			                    <a href="javascript:void(0);" onclick="editSNS('113');">save</a> | <a href="javascript:void(0);" onclick="delSNS('113','2');">del</a>
			                </td>
			            </tr>
			           <tr>
			                <td colspan="2" class="lineOnTop">
			                <textarea rows="5" cols="20" onblur="this.className='detailedViewTextBox'" name="serialnumbers2" id="serialnumbers2" onfocus="this.className='detailedViewTextBoxOn'" tabindex="" class="detailedViewTextBox"></textarea>
			                </td>
			            </tr>
			        </tbody>
			    </table>
