...
Vtiger_Edit_Js("Inventory_Edit_Js", {
    ...
    lineItemAddSNPopOverTemplate : '<div class="popover lineItemPopoverAddSN" role="tooltip"><div class="arrow"></div>\n\
                                <h3 class="popover-title">Serial Number</h3>\n\
...
//						}
	...
	registerAddSNShowEvent : function(){

		var self = this;
		
		this.lineItemsHolder.on('click', '#addSN', function(e){
			alert();return false;
			var element = jQuery(e.currentTarget);
			var response = self.isProductSelected(element);
			if(response == false){
			...
			element.popover('destroy');
            var lineItemRow = self.getClosestLineItemRow(element);
			self.getForm().find('.popover.lineItemPopoverAddSN').css('opacity', 0).css('z-index', '-1');
...
//						}
			...
			var AddSNUI = parentElem.find('div.AddSN').clone(true, true).removeClass('hide').addClass('show');
            var template = jQuery(Inventory_Edit_Js.lineItemAddSNPopOverTemplate);
            template.addClass('AddSNForm');
            var productTotal = self.getLineItemTotal(lineItemRow);
            var popOverTitle = AddSNUI.find('.popover_title').find('.variable').text(productTotal).closest('.popover_title').text();
			element.popover({
                'content' : AddSNUI,
                'html' : true,
                'placement' : 'top',
            ...
            element.one('shown.bs.popover', function(e) {
				callBackFunction(element, jQuery('.AddSNForm'));
				if(element.next('.popover').find('.popover-content').height() > 300) {
					app.helper.showScroll(element.next('.popover').find('.popover-content'), {'height': '300px'});
				}
            })
