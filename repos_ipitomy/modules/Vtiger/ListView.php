...
// Travis - Search invoice's serial number BEGIN
if($currentModule == 'Invoice' && ($_REQUEST['search_field']=='search_serial_number' || strpos($ustring,'search_serial_number')!==false)) {
	$queryArr = explode('where',strtolower($list_query));
	$serialQuery = " LEFT JOIN product_serial_number ON product_serial_number.invoiceid = vtiger_invoice.invoiceid";
	$serialWhere = " AND product_serial_number.serial_number LIKE '%".mysql_real_escape_string($_REQUEST['search_text'])."%'";
	
	$list_query = $queryArr[0].$serialQuery." WHERE ".$queryArr[1].$serialWhere." GROUP BY vtiger_invoice.invoiceid";
...
// Travis - Search invoice's serial number END
...
if(PerformancePrefs::getBoolean('LISTVIEW_COMPUTE_PAGE_COUNT', false) === true) {
	if($currentModule == 'Invoice' && ($_REQUEST['search_field']=='search_serial_number' || strpos($ustring,'search_serial_number')!==false)) {
		$count_result = $adb->query( $list_query);
