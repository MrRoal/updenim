...
<!--				 <td width="2%"><input type="checkbox" NAME="selected_id" id="{$entity_id}" value= '{$entity_id}' onClick=check_object(this); toggleSelectAll(this.name,"selectall")></td>-->
				 <td width="2%">&nbsp;</td>
				 {foreach key=index item=data from=$entity}	
				 	{if $index eq 6}
						<td id="mouseArea_{$entity_id}_{$EVENTID}_Status" onmouseover="hndMouseOverVTCall(55,'Status','{$entity_id}','{$EVENTID}');" onmouseout="fnhide('crmspanid');" width="17%">
				 <span id="dtlview_{$entity_id}_{$EVENTID}_Status">{$data}</span>
						<div id="editarea_{$entity_id}_{$EVENTID}_Status" style="display:none;">
								<input class="detailedViewTextBox" onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'" type="text" id="txtbox_{$entity_id}_{$EVENTID}_Status" name="eventstatus" maxlength='100' value="{$data|strip_tags|trim}" style="width:125px">
								 
								<input name="button_Status" type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="dtlViewAjaxSaveVTCallPotential('Status','Calander',55,'vtiger_activity','eventstatus','{$entity_id}','{$EVENTID}');fnhide('crmspanid');"/>
								{$APP.LBL_OR}
								<a href="javascript:;" onclick="hndCancel('dtlview_{$entity_id}_{$EVENTID}_Status','editarea_{$entity_id}_{$EVENTID}_Status','Status')" class="link">
