...
function vtcall_check_form(eventid)
{
		...
		if(dateComparison('due_date','End date','date_start','Start date','GE')) {
			...
			if (document.EditView.followup.checked == true &&
				...
				if(document.EditView.activitytype[0].checked == true) {
					date3.setMinutes(parseInt(date3.getMinutes(),10)+15);
				}
...
{	
	var startdate = document.getElementById(start);
		var dateelements3=splitDateVal(startdate.value)
		dd3=parseInt(dateelements3[0],10)+1;
        mm3=dateelements3[1]
        yyyy3=dateelements3[2]
        ...
        document.getElementById('jscal_field_followup_date').value=tempdate;
}
...
function changeEndtime_StartTimeVTCall(type)
{
	type=getObj('activitytype').value;
	calDuedatetimeVTCall(type);
}
 
function calDuedatetimeVTCall(type)
{		
        var dateval1=getObj('date_start').value.replace(/^\s+/g, '').replace(/\s+$/g, '');        
        var dateelements1=splitDateVal(dateval1);
	...
	if(type == 'Call')
        {
        	var followup_date=splitDateVal(date);
        	ddf=parseInt(followup_date[0],10)+1;
	        mmf=followup_date[1]
	        yyyyf=followup_date[2]
	
	        var datefol=new Date()
			datefol.setYear(yyyyf)
	        datefol.setMonth(mmf-1)
	        datefol.setDate(ddf)
	        var tempdate_follow = getdispDate(datefol);
			...
			hour = _2digit(hour);
                        ...
                        document.EditView.endmin.value = min;
                        document.EditView.endfmt.value = fmt;
			document.EditView.followup_date.value = tempdate_follow;
                        document.EditView.followup_starthr.value = '09';
                        document.EditView.followup_startmin.value = '00';
                        document.EditView.followup_startfmt.value = 'am';
                }else if(fmt == 'am')
		{
                        if(hour == 11 && min == 55)
                        {
...
function getdispDate(tempDate,datefmt)
{	
