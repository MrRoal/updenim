...
//		bundles_qty
		$other->list_fields['Qty/Unit']=Array('products'=>'qty_per_unit');
		$other->list_fields_name['Qty/Unit']='qty_per_unit';
...
//echo $query."<br>";
		...
		foreach ($return_value['entries'] as $key=>$val) {			
			foreach($val as $fIndex => $fValue) {
				
				if ($fIndex == 3) {
					
					$realVal = trim(strip_tags($fValue));
					$label = trim(strip_tags($return_value['header'][$fIndex]));
					$label = str_replace(' ', '_', $label);
					
					$wrapper = "<div ";
					$wrapper .= " id=\"mouseArea_$label"."_".$key."\" onmouseover=\"hndMouseOver(1,'$label"."_".$key."');\" onmouseout=\"fnhide('crmspanid');\"> ";
					$wrapper .= " &nbsp;&nbsp;<span id=\"dtlview_$label"."_".$key."\">$fValue</span> ";
					$wrapper .= " <div id=\"editarea_$label"."_".$key."\" style=\"display:none;\"> ";
					$wrapper .= "<input class=\"detailedViewTextBox\" onFocus=\"this.className='detailedViewTextBoxOn'\" onBlur=\"this.className='detailedViewTextBox'\" type=\"text\" id=\"txtbox_$label"."_".$key."\" name=\"bundles_qty\" maxlength=\"100\" value=\"$realVal\" style=\"width:80px;\"></input>";		
					$wrapper .= "<br><input name=\"button_$label\" type=\"button\" class=\"crmbutton small save\" value=\"Save\" onclick=\"dtlViewAjaxSave('$label"."_".$key."','Products',1,'vtiger_seproductsrel','bundles_qty::$id','$key');fnhide('crmspanid');\"/> OR
	                                    <a href=\"javascript:;\" onclick=\"hndCancel('dtlview_$label"."_".$key."','editarea_$label"."_".$key."','$label"."_".$key."')\" class=\"link\">Cancel</a>
	                                </div>";
					$wrapper .= "</div>";
					$fValue = $wrapper;
					$return_value['entries'][$key][$fIndex] = $fValue;
				}
			}
		}
		...
		foreach($with_crmids as $with_crmid) {
			if($with_module == 'Leads' || $with_module == 'Accounts' ||
				...
				if($adb->num_rows($query)==0){
					$adb->pquery("insert into vtiger_seproductsrel (crmid, productid, setype) values (?,?,?)", array($with_crmid, $crmid, $with_module));
				}
			}
			else {
