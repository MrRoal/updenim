...
function set_return_inventory(product_id,product_name,unitprice,qtyinstock,taxstr,curr_row,desc,subprod_id) {
	...
	if(window.opener.gVTModule == 'Invoice') {
		// auto populate bundle items
    	autoPopulateProductBundles(product_id, curr_row);
	}
...
function autoPopulateProductBundles(product_id, curr_row) {
    ...
    while(has_associate) {
        var ar = new Ajax.Request(
            ...
            {queue: {position: 'end', scope: 'command'},
                method: 'post',
                postBody: 'module=Products&action=ProductsAjax&file=ProductAssociate&product_id=' + product_id+'&ptype=Products&cnt='+count+'&ignore_ids='+ignore_ids+'&modulerel='+window.opener.gVTModule,
                asynchronous: false
            }
        );
        var res = ar.transport.responseText;      
        
        var a_temp = res.split('###');
        if(a_temp[0] > 0) {
            var prev = tableName.rows.length;            
            var count_ = eval(prev)-1;
            var row = tableName.insertRow(prev);
            row.id = "row"+count_;
            row.style.verticalAlign = "top";
            row.innerHTML = a_temp[2];

            if(ignore_ids == '') {            
            	window.opener.jQuery('input[name="subproduct_ids'+curr_row+'"]').parent().parent().parent().find('tr:gt(0):last').remove();
            }
            ignore_ids += a_temp[0] + ',';
            
            count++;
            //window.opener.document.EditView.elements["listPrice"+curr_row].value = '0.00';
    		//window.opener.document.EditView.elements["qty"+curr_row].value = '1';
        } else {
            has_associate = false;
        }
