...
function fnAddProductRow(module,image_path){
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text">'+
						...
						'</td></tr><tr><td class="small" id="setComment'+count+'"><textarea id="comment'+count+'" name="comment'+count+'" class=small style="width:70%;height:40px"></textarea><img src="themes/images/clear_field.gif" onClick="getObj(\'comment'+count+'\').value=\'\'"; style="cursor:pointer;" /></td></tr><tr><td><input type="button" value="Add SNs" class="crmbutton small save" onclick="showSN(this, '+count+');"/><div id="serial_number_'+count+'" class="discountUI"></div></td></tr></tbody></table>';	
...
function showSN(obj, num_row){
	var product_id=jQuery('#hdnProductId'+num_row).val();
    ...
    .done(function (html) {
        jQuery('#serial_number_'+num_row).html(html);        
        fnvshobj(obj,'tax_container');
		document.getElementById('serial_number_'+num_row).style.left =  document.getElementById("tax_container").left;
		document.getElementById('serial_number_'+num_row).style.top = document.getElementById("tax_container").top;
		document.getElementById('serial_number_'+num_row).style.display = "block";
    });
}

function editSNS(id) {
    jQuery('#c_vtbusy_info').css('display', 'inline');
	var val=jQuery('#serial_number'+id).val();
	jQuery.ajax({
        type: "POST",
        url: "index.php",
        data: { 
...
function addSNS(num_row) {
    jQuery('#c_vtbusy_info').css('display', 'inline');
	var product_id=jQuery('#hdnProductId'+num_row).val();
	var invoice_id=jQuery('#invoice_id').val();
	var serialnumbers=jQuery('#serialnumbers'+num_row).val();
	jQuery.ajax({
        type: "POST",
        url: "index.php",
        data: { 
...
function addSNSAdvance(num_row, invoice_id, product_id) {
    jQuery('#c_vtbusy_info').css('display', 'inline');
    var serialnumbers=jQuery('#serialnumbers'+num_row+product_id).val();
    jQuery.ajax({
        type: "POST",
        url: "index.php",
        data: {
