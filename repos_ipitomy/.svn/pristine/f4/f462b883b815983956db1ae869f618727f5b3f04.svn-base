...
function getDetailAssociatedProducts($module, $focus) {
	...
	$output = '';
	//Header Rows
    // Travis - add button "Add SNs" on invoice module
    if($module=='Invoice'){
        $output .= '<div id="tax_container" style="display:none; position:absolute; z-index:1px;"></div>';
        $output .= '<input name="invoice_id" id="invoice_id" type="hidden" value="'.$focus->id.'" />';
    }
		...
		$output .= '
			   <tr valign="top">
				<td class="crmTableRow small lineOnTop">
					...
					<br>' . $comment;
        // Travis - add button "Add SNs on invoice module"
        if($module=='Invoice' && $entitytype == 'Products' && $subprodname_str == ''){
            $output .= '<br/><input type="button" value="Add SNs" class="crmbutton small save" onclick="showSN(this, \''.$i.'\');" />';
            $output .= '<div class="discountUI" id="serial_number_'.$i.'"></div>';
            $output .= '<input type="hidden" value="'.$productid.'" name="hdnProductId'.$i.'" id="hdnProductId'.$i.'">';
        }
        $output .= '</td>';
